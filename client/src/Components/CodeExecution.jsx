...
//  const [ProblemID, setProblemID] = useState('');
  ...
  const fetchTestCasesForProblem = async () => {
    try {
      ...
      const response = await fetch(`${backendUrl}/testcasesforProblem/${ProblemId}`);
      if (response.ok) {
  ...
  return (
    <div>
        <Container>
            <div className='d-flex m-0 p-0'><p className='m-0'>Write your code in C++</p></div>
            <div className='w-100 '>
                <textarea className='w-100'  style={{height:'55vh'}} placeholder="Enter your code here" value={code} onBlur={handleCodeBlur} onChange={(e) => setCode(e.target.value)}></textarea>
                {submissionStatus ? <p>{submissionStatus}</p> : <></>}
            </div>
            <div className='overflow-auto text-wrap w-100 mt-0 border' style={{ height: '18vh' }}>
